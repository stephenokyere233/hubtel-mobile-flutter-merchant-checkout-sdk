...
class CheckoutEndPoint with EndPointCore {
  ...
  Future<ApiEndPoint> setupDeviceForBankPayment({required SetupPayerAuthRequest requestBody}){
    return createEndpoint(
        authority: _basePath,
  ...
  Future<ApiEndPoint> makeEnrollment({required String transactionId}){
    return createEndpoint(
        authority: _basePath,
        path: "api/v1/merchant/$merchantId/cardnotpresent/enroll-payerauth/$transactionId",
        requestType: HttpVerb.GET
    );
  }
}
